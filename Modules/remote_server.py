...
def sftpClient(
        _ligandListFilePath,
        _remotepath='/home/export/online1/mdt00/shisuan/swyaow/vina_result/',
        _hostname='xx.xxx.x.xx',
        _port=22,
        _username='xxxx',
        _passwd='xxxxxxxxx'
):
        ...
        try:
            if not os.path.exists(getResultsubsFile):
                if not os.path.exists(subsFile):
                    ...
                    sftp.put(os.path.join(os.path.dirname(currentPath), 'vina', 'bsub.sh'), os.path.join(remoteServerTaskPath, 'bsub.sh'))
                    sshClient.exec_command('chmod +x ' + os.path.join(remoteServerTaskPath, 'bsub.sh'))
